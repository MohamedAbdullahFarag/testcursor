...
public class AuditServiceTests
{
    ...
    public async Task GetUserAuditLogsAsync_ShouldReturnUserSpecificLogs_WhenRepositoryReturnsData()
    {
        ...
        _mockAuditLogRepository.Setup(r => r.GetUserAuditLogsAsync(userId, filter)).ReturnsAsync(repositoryResult);
        _mockMapper.Setup(m => m.Map<List<AuditLogDto>>(repositoryResult.Items)).Returns(expectedDtos);
        ...
        _mockAuditLogRepository.Verify(r => r.GetUserAuditLogsAsync(userId, filter), Times.Once);
    }

    [Test]
    public async Task GetSecurityEventsAsync_ShouldReturnSecurityEvents_WhenRepositoryReturnsData()
    {
        // Arrange
        var fromDate = DateTime.UtcNow.AddDays(-7);
        var toDate = DateTime.UtcNow;

        var securityEvents = new List<AuditLog>
        {
            new AuditLog { AuditLogId = 1, Action = "LOGIN_FAILED", Category = AuditCategory.Security },
            new AuditLog { AuditLogId = 2, Action = "ACCOUNT_LOCKED", Category = AuditCategory.Security }
        };

        var expectedDtos = new List<AuditLogDto>
        {
            new AuditLogDto { AuditLogId = 1, Action = "LOGIN_FAILED", Category = AuditCategory.Security },
            new AuditLogDto { AuditLogId = 2, Action = "ACCOUNT_LOCKED", Category = AuditCategory.Security }
        };

        _mockAuditLogRepository.Setup(r => r.GetSecurityEventsAsync(fromDate, toDate)).ReturnsAsync(securityEvents);
        _mockMapper.Setup(m => m.Map<IEnumerable<AuditLogDto>>(securityEvents)).Returns(expectedDtos);
        ...
        Assert.That(result.All(e => e.Category == AuditCategory.Security), Is.True);
        _mockAuditLogRepository.Verify(r => r.GetSecurityEventsAsync(fromDate, toDate), Times.Once);
    }

    [Test]
    public async Task ExportAuditLogsAsync_ShouldReturnExportedData_WhenValidFormatProvided()
    {
        // Arrange
        var filter = new AuditLogFilter
        {
        ...
        var format = AuditLogExportFormat.CSV;
        ...
        // Act
        var result = await _auditService.ExportAuditLogsAsync(filter, format);
    ...
    public async Task ArchiveOldLogsAsync_ShouldReturnArchivedCount_WhenValidRetentionPeriodProvided()
    {
        ...
        _mockAuditLogRepository.Setup(r => r.ArchiveOldLogsAsync(retentionPeriod)).ReturnsAsync(expectedCount);
        ...
        _mockAuditLogRepository.Verify(r => r.ArchiveOldLogsAsync(retentionPeriod), Times.Once);
    }
    ...
    public async Task VerifyLogsIntegrityAsync_ShouldReturnIntegrityResults_WhenValidDateRangeProvided()
    {
        ...
        _mockAuditLogRepository.Setup(r => r.VerifyLogsIntegrityAsync(fromDate, toDate)).ReturnsAsync(integrityResults);

        // Act
        var result = await _auditService.VerifyLogsIntegrityAsync(fromDate, toDate);
        ...
        _mockAuditLogRepository.Verify(r => r.VerifyLogsIntegrityAsync(fromDate, toDate), Times.Once);
    }
