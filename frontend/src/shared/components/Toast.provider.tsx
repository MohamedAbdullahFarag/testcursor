...
const ToastItem: React.FC<ToastItemProps> = ({ toast, onRemove }) => {
  ...
  const getToastStyles = () => {
    const baseStyles = 'rounded-lg shadow-lg p-4 max-w-sm w-full';
...
export const useToastHelpers = () => {
  ...
  return {
    showSuccess: (message: string, options?: Partial<Omit<Toast, 'id' | 'type' | 'message'>>) =>
      addToast({ type: 'success', message, ...options }),
    
    showError: (message: string, options?: Partial<Omit<Toast, 'id' | 'type' | 'message'>>) =>
      addToast({ type: 'error', message, ...options }),
    
    showWarning: (message: string, options?: Partial<Omit<Toast, 'id' | 'type' | 'message'>>) =>
      addToast({ type: 'warning', message, ...options }),
    
    showInfo: (message: string, options?: Partial<Omit<Toast, 'id' | 'type' | 'message'>>) =>
